...
def main(csv_file):
    ...
    *** This function was created with the help of ChatGPT 4 ***
    '''
    ...
    '''Markers that are dots
    for item in data:
        folium.CircleMarker(
            location=[float(item['Latitude']), float(item['Longitude'])],
            radius=5,  # Specifies the radius of the circle in pixels
            color='red',  # Color of the circle
            fill=True,
            fill_color='red',  # Fill color inside the circle
            fill_opacity=0.6,  # Fill opacity
            popup=folium.Popup(f"{item['ImageID']}<br>Latitude: {item['Latitude']}<br>Longitude: {item['Longitude']}", max_width=450)
        ).add_to(folium_map)'''
